...
**Claude Code (Claude Opus 5)** in the Claude desktop app, for the whole project: reading the brief, proposing designs, writing the code, tests, scripts and docs, running them, and clicking through the UI in its built-in browser. Project configuration (TypeScript, ESLint, Tailwind, Next.js) comes from my own Next.js + tRPC + Prisma boilerplate starter.
...
- **Project structure: I overruled the recommendation.** Claude's first cut folded the repository and service layers into the handlers and routers, arguing a slice this small doesn't need five files. I hold my boilerplate's standard: `schema → repository → handlers → service → router` inside `features/<slice>/server/`, an `api/v1` gateway, `providers/`, `components/{layout,shared,ui}`, `T`-prefixed types, and the `apiResponse` envelope on every reply. The code follows it now; the only differences left from the starter are the ones the brief forces (SQLite, no real auth, real-database tests).
...
1. `npm test`: **27 tests** on a throwaway SQLite database. They cover the brief's exact race sequence (A never touches the card), both parents paying at once (loser voided, never captured), 10 payers who all passed the pre-check (exactly one seat), double-click Pay (charged once), duplicates, declines and retries, ownership, staff-only rosters, HTTP status codes, and direct database writes that the constraints must reject. Every test also asserts the global invariants afterwards.
2. **Sabotage runs:** I removed the capacity guard, the pre-charge check, the capture/void decision, the ownership check and the duplicate check, one at a time, and confirmed each breaks the matching tests. With the capacity guard removed, the database `CHECK` still refused the 5th seat.
...
8. **Fresh clone:** `npm ci && npm run verify` in an empty directory.
